...
class _RepLoginPageState extends State<RepLoginPage> {
  ...
  Future<void> _openSecretDialog() async {
    final result = await showDialog<_TempPasswordResult>(
      context: context,
...
class _TempPasswordResult {
  final String email;
  final bool ok;

  const _TempPasswordResult({required this.email, this.ok = true});
}
...
class _TempPasswordDialogState extends State<_TempPasswordDialog> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      Navigator.of(context).pop(_TempPasswordResult(email: email));
    } catch (e) {
      if (!mounted) return;
