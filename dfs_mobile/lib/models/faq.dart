...
class FaqCategory {
  ...
  factory FaqCategory.fromJson(Map<String, dynamic> json) {
    Map<String, String> _intlMap(dynamic raw) {
      if (raw is Map) {
        return raw.map((key, value) {
          ...
          if (lang.isEmpty || text == null || text.isEmpty) {
    ...
    final preferredLang =
        _normLang(json['lang']?.toString() ?? json['language']?.toString());
    ...
    if (fallbackTitle.isNotEmpty &&
        ...
        !titleIntl.containsKey(preferredLang)) {
      titleIntl[preferredLang] = fallbackTitle;
    }
    if (fallbackDescription.isNotEmpty &&
        preferredLang.isNotEmpty &&
        !descriptionIntl.containsKey(preferredLang)) {
      descriptionIntl[preferredLang] = fallbackDescription;
    }

    final resolvedTitle =
        _resolveIntl(titleIntl, fallbackTitle, preferredLang).trim();
    final resolvedDescription =
        _resolveIntl(descriptionIntl, fallbackDescription, preferredLang)
            .trim();
    final descriptionValue =
        resolvedDescription.isNotEmpty ? resolvedDescription : fallbackDescription;
  ...
  String localizedDescription(String lang) =>
      _resolveIntl(descriptionIntl, description ?? '', lang);
}
