...
function setCors(res) {
  ...
  res.setHeader('Access-Control-Allow-Methods', 'POST,OPTIONS'); // wir antworten nur auf POST/OPTIONS
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Gate, X-Debug, X-Rep-Secret, X-Admin-Secret');
  res.setHeader('Access-Control-Max-Age', '600');
  res.setHeader('Content-Type', 'application/json; charset=utf-8');
}

// ---------- Upstash (gleich wie /api/rep/decision.js) ----------
const redisUrl =
...
// gleiche Präfixe / Index wie decision.js
...
// ticket -> { key, c }  (Direkt, Index, SCAN-Fallback)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // *** Nur Vertreter-Felder zurücksetzen – Admin-Felder UNVERÄNDERT ***
    ...
    // Speichern – identisch zu decision.js
    await redis.set(key, c);

    // Audit (Best-Effort, 7 Tage TTL)
    try {
      await redis.set(`dfs:audit:repDecisionReset:${ticket}:${nowIso()}`,
