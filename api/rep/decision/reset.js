...
// --------------- CORS ------------------
// Wichtig: sofort setzen und die vom Browser angeforderten Header spiegeln
function setCors(req, res) {
  const PROD_FE = process.env.WEB_ORIGIN || 'https://dfs-complaints-web.vercel.app';
  const origin = req.headers?.origin || '';
  const isPreview = /^https:\/\/dfs-complaints-web-[a-z0-9-]+(?:-[a-z0-9-]+)?\.vercel\.app$/i.test(origin);
  const isLocal = origin?.startsWith('http://localhost');
  const allowOrigin = origin && (origin === PROD_FE || isPreview || isLocal) ? origin : PROD_FE;

  // vom Browser gesendete Wunsch-Header übernehmen (macht Preflight robuster)
  const acrh = req.headers['access-control-request-headers'];
  const allowHeaders = acrh && typeof acrh === 'string'
    ? acrh
    : 'Content-Type, Authorization, X-Gate, X-Debug, X-Rep-Secret, X-Admin-Secret';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // *** Nur Vertreter-Felder zurücksetzen – Admin-Felder bleiben unverändert ***
    ...
    // Audit (Best Effort, 7 Tage)
    try {
      await redis.set(
        `dfs:audit:repDecisionReset:${ticket}:${nowIso()}`,
        { by: repId, at: nowIso(), action: 'reset' },
        { ex: 60 * 60 * 24 * 7 }
      );
    } catch (e) {
