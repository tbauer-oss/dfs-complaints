...
import { portalUserByEmail, portalUserSave, sanitizeTilePermissions, StoreInfraError } from '../_lib/store.js';
import {
  ADMIN_EMAILS,
  bootstrapInitialAdminsInBackground,
  normalizeRole,
  normalizeStatus,
  PORTAL_ROLES,
...
function mapStoreError(err) {
  if (err instanceof StoreInfraError) {
    if (err.code === 'RATE_LIMITED') {
      return {
        statusCode: 429,
        code: 'RATE_LIMITED',
        reason: 'STORE_LIMIT',
        message: 'Service temporär limitiert. Bitte später erneut versuchen.',
      };
    }
    return {
      statusCode: 503,
  ...
  if (raw.includes('max requests limit exceeded') || raw.includes('rate limit')) {
    return {
      statusCode: 429,
      code: 'RATE_LIMITED',
      reason: 'STORE_LIMIT',
...
function startBootstrapAdmins() {
  bootstrapInitialAdminsInBackground(console);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let u = null;
    try {
      u = await portalUserByEmail(rawEmail || email);
    } catch (err) {
      const mapped = mapStoreError(err);
      authLog(mapped.reason, {
        emailHash,
        step: 'lookup_user',
        error: err?.message || 'store error',
      });
      return authError(req, res, mapped.statusCode, mapped.code, mapped.message, mapped.reason);
    }
    ...
    if (ADMIN_EMAILS.has(email) && adminSecret && passwordOptions.includes(adminSecret)) {
      ...
      authLog('OK', { emailHash, role: auth.profile.role, via: 'admin_secret' });
      return ok(res, {
        ok: true,
        token: auth.token,
        user: auth.profile,
    ...
    if (!u) {
      authLog('USER_NOT_FOUND', { emailHash });
    ...
    if (!verification.okPw) {
      authLog('PASSWORD_MISMATCH', { emailHash });
    ...
    if (verification.usedPlaintextFallback) {
      ...
      try { await portalUserSave(u); } catch {}
