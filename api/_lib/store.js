...
const KEY_TRAINING_PROGRAM_NEED_LINKS = `${P}training:program:need-links`;
...
export async function createPortalUser(u) {
  ...
  const isSales = u?.isSales === true;
  const isPRRC = u?.isPRRC === true;
  const assignedDepartments = normalizeDepartments(u?.assignedDepartments || []);
...
function normalizeGsprSourceMeta(input = {}) {
  ...
  return {
    ...
    sourceUrl: _text(input.sourceUrl, 500),
    parserVersion: _text(input.parserVersion, 80),
    normalizedText: _text(input.normalizedText, 250000),
    lastSyncAt: parseDate(input.lastSyncAt),
    lastGoodSyncAt: parseDate(input.lastGoodSyncAt),
    lastAttemptAt: parseDate(input.lastAttemptAt),
    lastSyncAttemptAt: parseDate(input.lastSyncAttemptAt),
    lastError: _text(input.lastError, 1000),
    lastSyncError: _text(input.lastSyncError, 1000),
    lastFailureReason: _text(input.lastFailureReason, 240),
...
const TRAINING_NEED_STATUSES = ['draft', 'submitted', 'managerApproved', 'glApproved', 'scheduled', 'integrated', 'closed'];
const TRAINING_PROGRAM_STATUSES = [
  'planned',
  'inProgress',
  'completed',
  'cancelled',
...
function normalizeTrainingNeed(record = {}) {
  ...
  const recurrenceIntervalMonths = Number(record.recurrenceIntervalMonths || 0) || null;
  const nextDuePeriod = normalizeString(record.nextDuePeriod || '');
  const recurrenceActive = record.recurrenceActive === false ? false : true;
  const lastGeneratedAt = Number(record.lastGeneratedAt || 0) || null;
  const parentRecurringId = normalizeString(record.parentRecurringId || '');
  const isAutoGenerated = Boolean(record.isAutoGenerated);
  const integratedAt = Number(record.integratedAt || 0) || null;
  const integratedByUserId = normalizeString(record.integratedByUserId || record.integratedBy || '');
  return {
    ...
    integratedAt,
    integratedByUserId: integratedByUserId || null,
    recurrenceIntervalMonths,
    nextDuePeriod: nextDuePeriod || null,
    recurrenceActive,
    lastGeneratedAt,
    parentRecurringId: parentRecurringId || null,
    isAutoGenerated,
...
function normalizeTrainingProgram(record = {}) {
  const year = Number(record.year || record.trainingYear || new Date().getFullYear());
  const status = TRAINING_PROGRAM_STATUSES.includes(record.status) ? record.status : 'planned';
  const plannedPeriodType = normalizeString(record.plannedPeriodType || '');
  const plannedPeriodValue = normalizeString(record.plannedPeriodValue || '');
  const format = normalizeString(record.format || record.trainingFormat || '');
  const responsiblePerson = normalizeString(record.responsiblePerson || record.owner || '');
  const participantsPlanned = normalizeString(record.participantsPlanned || record.participants || '');
  const targetGroup = normalizeString(record.targetGroup || '');
  const category = normalizeString(record.category || '');
  const categoryFreeText = normalizeString(record.categoryFreeText || record.categoryFree || '');
  const intervalType = normalizeString(record.intervalType || '');
  const intervalValue = normalizeString(record.intervalValue || '');
  const trainerProvider = normalizeString(record.trainerProvider || record.trainer || record.provider || '');
  const location = normalizeString(record.location || record.meetingLink || '');
  const duration = normalizeString(record.duration || '');
  const notes = normalizeString(record.notes || record.comment || '');
  const cancellationReason = normalizeString(record.cancellationReason || record.reason || '');
  const needIdsSource = Array.isArray(record.needIds) ? record.needIds : Array.isArray(record.sourceNeedIds) ? record.sourceNeedIds : [];
  return {
    id: normalizeString(record.id || crypto.randomUUID()),
    year,
    title: normalizeString(record.title || `Schulungsprogramm ${year}`),
    status,
    owner: normalizeString(record.owner || record.coordinator || responsiblePerson || ''),
    department: normalizeString(record.department || record.team || ''),
    targetGroup,
    plannedPeriodType,
    plannedPeriodValue,
    format,
    ...
    intervalValue: intervalValue || null,
    category,
    categoryFreeText: categoryFreeText || null,
    responsiblePerson,
    participantsPlanned,
    trainerProvider,
    location,
    ...
    needIds: needIdsSource.map(normalizeString),
...
function normalizeTrainingProgramNeedLink(record = {}) {
  return {
    id: normalizeString(record.id || crypto.randomUUID()),
    programEntryId: normalizeString(record.programEntryId || record.programId || ''),
    trainingNeedId: normalizeString(record.trainingNeedId || record.needId || ''),
    linkedAt: Number(record.linkedAt || Date.now()),
    linkedByUserId: normalizeString(record.linkedByUserId || record.linkedBy || record.createdBy || ''),
  };
}

export async function trainingNeedGet(id) {
...
function normalizeTraining(record = {}) {
  ...
  return {
    ...
    categoryFreeText: normalizeString(record.categoryFreeText || record.categoryFree || ''),
    type: normalizeString(record.type || ''),
    ...
    startTime: normalizeString(record.startTime || ''),
    endTime: normalizeString(record.endTime || ''),
    durationMinutes: Number(record.durationMinutes || 0) || 0,
    trainer: normalizeString(record.trainer || record.provider || ''),
    trainerInternal: normalizeString(record.trainerInternal || (record.type === 'intern' ? record.trainer : '')),
    providerCompany: normalizeString(record.providerCompany || (record.type === 'extern' ? record.trainer : '')),
    location: normalizeString(record.location || record.link || ''),
    meetingLink: normalizeString(record.meetingLink || ''),
    ...
    ownerUserId: normalizeString(record.ownerUserId || record.owner || ''),
    ...
    intervalType: normalizeString(record.intervalType || ''),
    intervalValue: normalizeString(record.intervalValue || ''),
    recurrenceIntervalMonths: Number(record.recurrenceIntervalMonths || 0) || null,
    nextDueDate: normalizeString(record.nextDueDate || ''),
    recurrenceActive: record.recurrenceActive === false ? false : true,
    lastGeneratedAt: Number(record.lastGeneratedAt || 0) || null,
    parentRecurringId: normalizeString(record.parentRecurringId || '') || null,
    isAutoGenerated: Boolean(record.isAutoGenerated),
    ...
    notes: normalizeString(record.notes || record.agenda || ''),
...
async function trainingProgramNeedLinksRaw() {
  ...
  return mem.trainingProgramNeedLinks;
}
...
export async function trainingProgramGet(id) {
  const list = await trainingProgramsAll();
  const key = normalizeString(id || '');
  return list.find((entry) => entry.id === key) || null;
}

export async function trainingNeedDelete(id, { deleteInstances = false } = {}) {
  const list = await trainingNeedsAll();
  const next = list.filter((entry) => {
    if (entry.id === id) return false;
    if (deleteInstances && entry.parentRecurringId === id) return false;
    return true;
  });
...
export async function trainingProgramNeedLinksAll() {
  const list = (await trainingProgramNeedLinksRaw()) || [];
  const normalized = list.map(normalizeTrainingProgramNeedLink);
  mem.trainingProgramNeedLinks = normalized;
  await persistTrainingList(KEY_TRAINING_PROGRAM_NEED_LINKS, normalized);
  return normalized;
}
...
export async function trainingProgramNeedLinkSave(record = {}) {
  const list = await trainingProgramNeedLinksAll();
  const normalized = normalizeTrainingProgramNeedLink(record);
  if (!normalized.programEntryId || !normalized.trainingNeedId) return null;
  const existing = list.find(
    (entry) => entry.programEntryId === normalized.programEntryId && entry.trainingNeedId === normalized.trainingNeedId
  );
  if (existing) return existing;
  list.push(normalized);
  mem.trainingProgramNeedLinks = list;
  await persistTrainingList(KEY_TRAINING_PROGRAM_NEED_LINKS, list);
  return normalized;
}

export async function trainingProgramNeedLinkDelete(programEntryId, trainingNeedId) {
  const list = await trainingProgramNeedLinksAll();
  const next = list.filter(
    (entry) => !(entry.programEntryId === programEntryId && entry.trainingNeedId === trainingNeedId)
  );
  mem.trainingProgramNeedLinks = next;
  await persistTrainingList(KEY_TRAINING_PROGRAM_NEED_LINKS, next);
  return { removed: list.length - next.length };
}

export async function trainingTemplateGet(id) {
...
export async function trainingRecordErase(id, { deleteInstances = false } = {}) {
  const list = await trainingRecordsAll();
  const target = list.find((entry) => entry.id === id || entry.trainingNumber === id);
  if (!target) return { removed: 0 };
  const next = list.filter((entry) => {
    if (entry.id === target.id || entry.trainingNumber === target.trainingNumber) return false;
    if (deleteInstances && entry.parentRecurringId === target.id) return false;
    return true;
  });
  mem.trainingRecords = next;
  await persistTrainingList(KEY_TRAININGS, next);
  return { removed: list.length - next.length };
}

export async function trainingNeedsPurge() {
  const list = await trainingNeedsAll();
  mem.trainingNeeds = [];
  await persistTrainingList(KEY_TRAINING_NEEDS, []);
  mem.trainingProgramNeedLinks = [];
  await persistTrainingList(KEY_TRAINING_PROGRAM_NEED_LINKS, []);
  return { removed: list.length };
}

export async function trainingProgramsPurge() {
  const list = await trainingProgramsAll();
  mem.trainingPrograms = [];
  await persistTrainingList(KEY_TRAINING_PROGRAMS, []);
  mem.trainingProgramNeedLinks = [];
  await persistTrainingList(KEY_TRAINING_PROGRAM_NEED_LINKS, []);
  return { removed: list.length };
}

export async function trainingRecordsPurge() {
  const list = await trainingRecordsAll();
  mem.trainingRecords = [];
  await persistTrainingList(KEY_TRAININGS, []);
  return { removed: list.length };
}

export async function trainingTemplatesPurge() {
  const list = await trainingQuestionnaireTemplatesAll();
  mem.trainingQuestionnaireTemplates = [];
  await persistTrainingList(KEY_TRAINING_TEMPLATES, []);
  return { removed: list.length };
}

export async function trainingQuestionnairesPurge() {
  const list = await trainingQuestionnairesAll();
  mem.trainingQuestionnaires = [];
  await persistTrainingList(KEY_TRAINING_QUESTIONNAIRES, []);
  return { removed: list.length };
}
