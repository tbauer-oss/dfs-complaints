...
export default async function handler(req, res){
  ...
  try {
    ...
    // Pflichtfelder wie im Frontend
    if (!payload.article || !payload.desc) return bad(res,'required fields missing',400);
    if (payload.segment === 'Zahnarzt' && !payload.batch) return bad(res,'batch required for dentist',400);
    ...
    // ---- E-Mail & XML erst NACH erfolgreichem Speichern (dynamisch importieren)
