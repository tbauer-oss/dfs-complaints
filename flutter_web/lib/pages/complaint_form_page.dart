...
class ComplaintFormPage extends StatefulWidget {
  ...
  @override State<ComplaintFormPage> createState() => _ComplaintFormPageState();
}
...
class _ComplaintFormPageState extends State<ComplaintFormPage> {
  ...
  String applied = 'Nein'; String injury = 'Nein';
  ...
  String? info; String? err; bool busy=false;
  ...
  Widget build(BuildContext context) {
    ...
    final isDentist = segment == t.segment_dentist; // wir vergleichen lokalisierten Text
    final needInjuryDesc = isDentist && applied==t.yes && injury==t.yes;

    // Optionen lokalisiert setzen:
    ...
    // Falls Sprache umgeschaltet wird, sicherstellen, dass aktuelle Values existieren:
      ...
      child: Center(
        child: ConstrainedBox(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (isDentist) ...[
              const SizedBox(height: 8),
              TextField(controller: batch, decoration: InputDecoration(labelText: t.batch, border: const OutlineInputBorder())),
            ],
            const SizedBox(height: 8),
            Row(children: [
            ...
            DropdownButtonFormField(
              value: returned,
              items: [DropdownMenuItem(value: optReturnedYes, child: Text(optReturnedYes)),
            ...
            Row(children: [
              ...
              // Link-Button kannst du ggf. zusätzlich einbauen wie in Auth
            ]),
            ...
            ElevatedButton(
              onPressed: busy ? null : () async {
                ...
                if (article.text.isEmpty || desc.text.isEmpty){ err=t.required_fields; setState(()=>busy=false); return; }
                if (isDentist && batch.text.isEmpty){ err=t.batch; setState(()=>busy=false); return; }

                final res = await widget.api.submitComplaint({
                  'segment': segment == optDentist ? 'Zahnarzt' : 'Zahntechnik', // Backend erwartet DE-Labels; alternativ mappe serverseitig
                ...
                setState(()=>busy=false);
                if (res==null) { err=t.send_failed; }
                else { info=t.sent_ticket(res['ticket']); }
                setState((){});
              },
