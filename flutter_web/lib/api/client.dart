...
class ApiClient {
  ...
  Future<http.Response> register(Map<String,dynamic> body) =>
      http.post(Uri.parse('${CFG.apiBase}/api/auth/register'),
        headers: _headers(), body: jsonEncode(body));

  Future<bool> login(String email, String password) async {
    final r = await http.post(Uri.parse('${CFG.apiBase}/api/auth/login'),
  ...
  Future<List<dynamic>> myComplaints() async {
    final r = await http.get(Uri.parse('${CFG.apiBase}/api/complaint/mine'),
  ...
  Future<Map<String,dynamic>?> submitComplaint(
    ...
    for (final f in files) {
      req.files.add(http.MultipartFile.fromBytes('images', f.bytes, filename: f.name));
    }
